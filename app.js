...
// NOUVEAU : Fonction utilitaire pour échapper le HTML (prévention XSS)
...
// MODIFIED: Escape message before displaying in toast
...
// MODIFIED: Escape title before rendering in modal header
...
// MODIFIED: Escape title and message when showing confirm modal
function createAndShowModal(title, content, primaryButtonText, primaryButtonAction, cancelButtonText = 'Annuler', cancelButtonAction = 'closeModal()') {
    ...
    // The content is passed as contentHtml to showModal, so it needs to be safe.
    // Assuming 'content' parameter here is already HTML-safe or is raw text that needs escaping if used in showModal's title.
    // For confirm modal, message content is text, so it's escaped in showConfirmModal directly.
    showModal(title, content, buttons);
}

// Fonctions pour créer des éléments de formulaire - Ces fonctions génèrent du HTML avec des valeurs
// Les valeurs passées à ces fonctions (e.g., 'value') seront des strings d'input déjà échappées si elles viennent de l'utilisateur.
// Pour les labels et placeholders, ils sont généralement statiques ou contrôlés.
function createInput(id, label, type = 'text', value = '', placeholder = '', required = false, dataAttrs = {}) {
    ...
    // NEW: Escape value when setting it in the input to prevent attribute-based XSS (though value attr typically auto-escapes, explicit is safer for some contexts)
    // Note: This applies to the default value, user input in the field itself is not run as HTML.
    return `
        <div class="form-group">
            <label for="${id}">${escapeHTML(label)}${required ? ' *' : ''}</label>
            <input type="${type}" id="${id}" value="${escapeHTML(value)}" placeholder="${escapeHTML(placeholder)}" ${requiredAttr} ${dataAttributesString}>
        </div>
    `;
}
...
function createCheckboxGroup(name, label, options, selectedValues = [], idPrefix = '') {
    let checkboxesHtml = options.map(option => `
        <label>
            <input type="checkbox" id="${idPrefix}${escapeHTML(option.value)}" name="${name}" value="${escapeHTML(option.value)}" ${selectedValues.includes(option.value) ? 'checked' : ''}>
...
function createTextArea(id, label, value = '', placeholder = '', rows = 3) {
    // NEW: Escape value for textarea content
    return `
        <div class="form-group">
            <label for="${id}">${escapeHTML(label)}</label>
            <textarea id="${id}" placeholder="${escapeHTML(placeholder)}" rows="${rows}">${escapeHTML(value)}</textarea>
        </div>
    `;
}
...
function createDatePicker(id, label, value = '', required = false, dataAttrs = {}) {
    const requiredAttr = required ? 'required' : '';
    const dataAttributesString = Object.keys(dataAttrs).map(key => `data-${key}="${dataAttrs[key]}"`).join(' ');
    // NEW: Escape label for date picker
    return `
        <div class="form-group">
            <label for="${id}">${escapeHTML(label)}${required ? ' *' : ''}</label>
            <input type="date" id="${id}" value="${escapeHTML(value)}" ${requiredAttr} ${dataAttributesString}>
        </div>
    `;
}
...
// MODIFIED: Escape person.name when rendering the people list
function renderPeopleList() {
    ...
    people.forEach(person => {
        ...
        // NEW: Escape person.name for display in innerHTML
        li.innerHTML = `
...
function togglePersonVisibility(personId, buttonElement) {
    ...
    if (person) {
        ...
        showToast(`Visibilité de ${escapeHTML(person.name)} : ${person.isVisible ? 'Affichée' : 'Masquée'}.`, 'info');
    }
}
...
// MODIFIED: Escape person name before adding
async function addPerson() {
    ...
    const newPerson = {
        ...
        name: escapeHTML(name), // NEW: Escape name before storing
        color: null,
        isVisible: true
    };
...
function showEditPersonModal(personId) {
    ...
    `;
    createAndShowModal(
        `Modifier ${escapeHTML(person.name)}`, // NEW: Escape person.name in modal title
        content,
        'Sauvegarder',
...
// MODIFIED: Escape newName and person.name when updating events
async function editPerson(personId) {
    ...
    if (person) {
        ...
        person.name = escapeHTML(newName); // NEW: Escape newName before storing

        allCalendarEvents.forEach(event => {
            if (event.personId === person.id) {
                const eventTypeDisplay = getEventTypeDisplayName(event.type);
                // NEW: Escape person.name when updating event title
                event.title = `${escapeHTML(person.name)} (${escapeHTML(eventTypeDisplay)})`;
            }
        });
...
// MODIFIED: Escape person.name in confirm message
function confirmDeletePerson(personId) {
    ...
    createAndShowModal(
        ...
        `<p>Êtes-vous sûr de vouloir supprimer la personne "${escapeHTML(person.name)}" ? Tous les événements associés à cette personne seront également supprimés.</p>`,
        'Supprimer',
    ...
    try {
        ...
        if (storedEvents) {
            allCalendarEvents = storedEvents.map(event => {
                ...
                return {
                    ...event,
                    // NEW: Ensure titles loaded from DB are escaped if they weren't already
                    // (They should be if add/edit event correctly escaped them on save)
                    title: person ? `${escapeHTML(person.name)} (${escapeHTML(eventTypeDisplay)})` : `[Inconnu] (${escapeHTML(eventTypeDisplay)})`,
...
// MODIFIED: Implement eventContent hook for FullCalendar to escape event titles
function initFullCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDidMount: function(info) {
            // NEW: Escape event title for the tooltip
            info.el.title = `${escapeHTML(info.event.title)}`;
        },
        // NEW: CRITICAL FOR XSS - Escape event content when rendering HTML
        eventContent: function(eventInfo) {
            // FullCalendar's default rendering usually uses textContent, but
            // if event title contains HTML, it could be rendered.
            // Explicitly ensure the title is HTML-escaped when used in content.
            const escapedTitle = escapeHTML(eventInfo.event.title);
            // Default FullCalendar event content is usually just the title.
            // If custom HTML is being built here, ensure all parts from user input are escaped.
            return { html: `<div class="fc-event-main-frame">${escapedTitle}</div>` };
        },
...
function showAddPlanningEventModal(startStr = '', endStr = '') {
    ...
    // NEW: Ensure person names are escaped for display in select options
    const personOptions = people.map(p => ({ value: p.id, label: escapeHTML(p.name) }));
...
// MODIFIED: Escape event title when creating new events
async function addPlanningEvent() {
    ...
    const generateEvent = (start, end, recurrenceGroupId = null) => {
        ...
        return {
            id: crypto.randomUUID(),
            // NEW: Escape person.name and eventTypeDisplay when setting event title
            title: `${escapeHTML(person.name)} (${escapeHTML(eventTypeDisplay)})`,
            start: start,
            end: finalEnd,
            personId: person.id,
            type: eventType,
    ...
    updateCalendarEventsDisplay();
    closeModal();
    showToast(`Événement(s) pour ${escapeHTML(person.name)} ajouté(s) !`, 'success'); // NEW: Escape person.name
}
...
function showEditPlanningEventModal(eventId) {
    ...
    // NEW: Escape person names for select options
    const personOptions = people.map(p => ({ value: p.id, label: escapeHTML(p.name) }));
    const eventTypeOptions = [
        { value: 'permanence', label: 'Permanence' },
        { value: 'permanence_backup', label: 'Permanence (Backup)' },
...
// MODIFIED: Escape event title when editing an event
async function editPlanningEvent(eventId) {
    ...
    const updatedEvent = {
        ...
        // NEW: Escape person.name and eventTypeDisplay when updating event title
        title: `${escapeHTML(person.name)} (${escapeHTML(eventTypeDisplay)})`,
        start: startDate,
        end: finalEnd,
        personId: person.id,
...
// MODIFIED: Escape message in confirm modal
function confirmDeleteEvent(eventId) {
    createAndShowModal(
        'Confirmer la suppression',
        `<p>${escapeHTML('Êtes-vous sûr de vouloir supprimer cet événement ?')}</p>`,
        'Supprimer',
        `deleteEvent('${eventId}')`,
        'Annuler'
    );
...
function confirmDeleteRecurrenceSeries(recurrenceGroupId) {
    createAndShowModal(
        ...
        `<p>${escapeHTML('Êtes-vous sûr de vouloir supprimer toute la série d\'événements récurrents ?')}</p>`,
        'Supprimer la série',
...
function showImportModal() {
    const content = `
        ...
        <p>${escapeHTML('Attention: L\'importation écrasera les données existantes.')}</p>
    `;
    createAndShowModal(
    ...
    try {
        ...
        if (parsedData.people && Array.isArray(parsedData.people)) {
            people = parsedData.people.map(p => ({
                ...p,
                isVisible: p.isVisible !== undefined ? p.isVisible : true,
                name: escapeHTML(p.name) // NEW: Escape name on import
            }));
            for (const person of people) {
        ...
        if (parsedData.events && Array.isArray(parsedData.events)) {
            allCalendarEvents = parsedData.events.map(event => {
                ...
                return {
                    ...event,
                    // NEW: Ensure titles are re-escaped on import if they weren't already
                    title: person ? `${escapeHTML(person.name)} (${escapeHTML(eventTypeDisplay)})` : `[Inconnu] (${escapeHTML(eventTypeDisplay)})`,
                    backgroundColor: eventColor,
                    borderColor: eventColor,
                    allDay: true
...
function showExportOptionsModal(exportType) {
    ...
    const content = `
        <p>${escapeHTML('Génère un PDF listant les permanences par semaine sous forme de tableau (du Lundi au Vendredi).')}</p>
            ...
            while (day.isSameOrBefore(loopEndDate, 'day')) {
                ...
                if (dailyPermanences[dateKey]) { // S'assurer que le jour est dans la période d'export collectée
                    if (event.type === 'permanence') {
                        // NEW: Escape person.name before adding to Set for PDF data
                        dailyPermanences[dateKey].permanence.add(escapeHTML(person.name));
                    } else if (event.type === 'permanence_backup') {
                        // NEW: Escape person.name before adding to Set for PDF data
                        dailyPermanences[dateKey].permanence_backup.add(escapeHTML(person.name));
                    }
                }
                day = day.add(1, 'day');
            }
...
async function generatePermanencePdfTable(startDate, endDate) {
    ...
    if (loopStartDate.day() === 0) { // If start of week is Sunday (0), move to Monday (1)
        loopStartDate = loopStartDate.add(1, 'day');
    } else if (loopStartDate.day() !== 1) { // If not Sunday or Monday, go to previous Monday
    ...
    for (const week of relevantWeeksData) {
        ...
        week.forEach(dayData => {
            ...
            // Data from IndexedDB (permanenceNames, backupNames) is already escaped.
            // dayOfWeekFr is internally generated.
...
function showStatsModal() {
    ...
    const content = `
        <p>${escapeHTML('Calcule le nombre de jours de permanence par personne sur la période sélectionnée. Inclut les permanences et permanences (backup).')}</p>
...
// MODIFIED: Escape person name when displaying stats table
function displayStatsTable(stats, startDateStr, endDateStr) {
    ...
    let tableHtml = `
        <table class="stats-table">
            <thead>
                <tr>
                    <th>${escapeHTML('Personne')}</th>
                    <th>${escapeHTML('Jours de Permanence')}</th>
                </tr>
            </thead>
            <tbody>
    `;
    ...
    statsResultsDiv.innerHTML = tableHtml; // NEW: Set innerHTML
}
...
function exportStatsAsCsv(startDateStr, endDateStr) {
    ...
    // Headers
    const headers = Array.from(table.querySelectorAll('thead th')).map(th => escapeHTML(th.innerText)); // NEW: Escape headers
    csv.push(headers.join(';')); // Use semicolon for CSV (common in France)

    // Rows
    table.querySelectorAll('tbody tr').forEach(row => {
        const rowData = Array.from(row.querySelectorAll('td')).map(td => escapeHTML(td.innerText)); // NEW: Escape row data
